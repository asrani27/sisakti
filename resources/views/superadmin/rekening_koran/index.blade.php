...
@section('title', 'Rekening Koran - SI SAKTI')

@section('content')
<div class="content-area">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-white mb-2">Rekening Koran Perbulan</h1>
        <p class="text-gray-400">Rekening Koran Perbulan</p>
    </div>
    ...
    <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-4">
            <div class="relative">
                <input type="text" id="search" placeholder="Cari Rekening Koran..."
        ...
        <a href="{{ route('rekening_koran.create') }}"
    ...
    <div class="card-gradient rounded-xl overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody>
                    @forelse($rekeningKorans as $index => $rekeningKoran)
                    <tr class="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                        <td class="py-3 px-4 text-gray-200">{{ ($rekeningKorans->currentPage() - 1) * $rekeningKorans->perPage() + $index +
                            1 }}</td>
                        <td class="py-3 px-4 text-gray-200">{{ $rekeningKoran->skpd ? $rekeningKoran->skpd->nama_skpd : '-' }}</td>
                        <td class="py-3 px-4 text-gray-200">{{ $rekeningKoran->bulan }}</td>
                        <td class="py-3 px-4 text-gray-200">{{ $rekeningKoran->tahun }}</td>
                        <td class="py-3 px-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="showRekeningKoranDetails({{ $rekeningKoran->id }})"
                                    class="text-indigo-400 hover:text-indigo-300 transition-colors"
                                    title="Lihat Detail">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <a href="{{ route('rekening_koran.edit', $rekeningKoran->id) }}"
                                    class="text-blue-400 hover:text-blue-300 transition-colors" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="{{ route('rekening_koran.ocr', ['rekening_koran_id' => $rekeningKoran->id]) }}"
                                    class="text-emerald-400 hover:text-emerald-300 transition-colors" title="OCR PDF">
                                    <i class="fas fa-file-pdf"></i>
                                </a>
                                <form action="{{ route('rekening_koran.destroy', $rekeningKoran->id) }}" method="POST"
                    ...
                    <tr>
                        <td colspan="5" class="py-8 text-center text-gray-400">
                            ...
                            <p>Tidak ada data Rekening Koran</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <!-- Pagination -->
        @if($rekeningKorans->hasPages())
        <div class="mt-6">
            {{ $rekeningKorans->links() }}
        </div>
        @endif
    </div>
</div>

<!-- Modal Rekening Koran Details -->
<div id="rekeningKoranDetailModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-[9999]">
    <div class="bg-slate-900 rounded-xl shadow-2xl w-full max-w-6xl mx-4 max-h-[90vh] overflow-hidden">
        <div class="flex items-center justify-between p-6 border-b border-slate-700">
            <div>
                <h2 class="text-2xl font-bold text-white">Detail Rekening Koran</h2>
                <p id="rekeningKoranModalInfo" class="text-gray-400 mt-1"></p>
            </div>
            <button onclick="closeRekeningKoranModal()" class="text-gray-400 hover:text-white transition-colors">
                <i class="fas fa-times text-2xl"></i>
            </button>
        </div>
        <div class="p-6 overflow-y-auto max-h-[calc(90vh-150px)]">
            <div id="rekeningKoranDetailsContent">
...
<script>
    ...
    function showRekeningKoranDetails(rekeningKoranId) {
        const modal = document.getElementById('rekeningKoranDetailModal');
        const content = document.getElementById('rekeningKoranDetailsContent');
        const info = document.getElementById('rekeningKoranModalInfo');
        ...
        fetch(`/superadmin/rekening-koran/${rekeningKoranId}/details`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    info.textContent = `${data.rekeningKoran.skpd} - ${data.rekeningKoran.bulan} ${data.rekeningKoran.tahun}`;
    ...
    document.getElementById('rekeningKoranDetailModal').addEventListener('click', function(e) {
        if (e.target === this) {
