...
@section('title', 'SPJ Fungsional - SI SAKTI')

@section('content')
<div class="content-area">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-white mb-2">SPJ Fungsional</h1>
        <p class="text-gray-400">Surat Pertanggungjawaban Fungsional (Januari - Desember)</p>
    </div>
    ...
    <div class="flex justify-between items-center mb-6">
        <div class="flex gap-4">
            <div class="search-box">
                <input type="text" id="search" placeholder="Cari SPJ Fungsional..." 
        ...
        <a href="{{ route('upload.spj-fungsional.create') }}"
    ...
    <div class="card-gradient rounded-xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody id="table-body">
                    @forelse($spjFungsional as $index => $spj)
                    <tr class="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                        <td class="py-3 px-4 text-gray-200">{{ ($spjFungsional->currentPage() - 1) * $spjFungsional->perPage() +
                            $index + 1 }}</td>
                        ...
                        <td class="py-3 px-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('upload.spj-fungsional.edit', $spj->id) }}"
                                    class="text-blue-400 hover:text-blue-300 transition-colors" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="{{ route('upload.spj-fungsional.ocr', ['spj_fungsional_id' => $spj->id]) }}"
                                    class="text-emerald-400 hover:text-emerald-300 transition-colors" title="OCR PDF">
                                    <i class="fas fa-file-pdf"></i>
                                </a>
                                <form action="{{ route('upload.spj-fungsional.destroy', $spj->id) }}" method="POST"
                    ...
                    <tr>
                        <td colspan="5" class="py-8 text-center text-gray-400">
                            ...
                            <p>Tidak ada data SPJ Fungsional</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
