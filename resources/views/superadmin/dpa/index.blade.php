...
@section('title', 'DPA - SI SAKTI')

@section('content')
<div class="content-area">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-white mb-2">DPA Perbulan</h1>
        <p class="text-gray-400">Dokumen Pelaksanaan Anggaran Perbulan</p>
    </div>
    ...
    <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-4">
            <div class="relative">
                <input type="text" id="search" placeholder="Cari DPA..."
        ...
        <a href="{{ route('upload.dpa.create') }}"
    ...
    <div class="card-gradient rounded-xl overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody>
                    @forelse($dpas as $index => $dpa)
                    <tr class="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                        <td class="py-3 px-4 text-gray-200">{{ ($dpas->currentPage() - 1) * $dpas->perPage() + $index +
                            1 }}</td>
                        <td class="py-3 px-4 text-gray-200">{{ $dpa->skpd ? $dpa->skpd->nama_skpd : '-' }}</td>
                        <td class="py-3 px-4 text-gray-200">{{ $dpa->bulan }}</td>
                        <td class="py-3 px-4 text-gray-200">{{ $dpa->tahun }}</td>
                        <td class="py-3 px-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('upload.dpa.edit', $dpa->id) }}"
                                    class="text-blue-400 hover:text-blue-300 transition-colors" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="{{ route('upload.dpa.ocr', ['dpa_id' => $dpa->id]) }}"
                                    class="text-emerald-400 hover:text-emerald-300 transition-colors" title="OCR PDF">
                                    <i class="fas fa-file-pdf"></i>
                                </a>
                                <form action="{{ route('upload.dpa.destroy', $dpa->id) }}" method="POST"
