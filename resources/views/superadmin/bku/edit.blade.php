...
@section('title', 'Edit BKU - SI SAKTI')

@section('content')
<div class="content-area">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-white mb-2">Edit BKU</h1>
        <p class="text-gray-400">Edit Data Buku Kas Umum Perbulan</p>
    </div>

    <div class="card-gradient rounded-xl p-6">
        <form action="{{ route('upload.bku.update', $bku->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="mb-4">
                    ...
                    <select id="kode_skpd" name="kode_skpd"
                        ...
                        <option value="{{ trim($skpd->kode_skpd) }}" {{ old('kode_skpd', trim($bku->kode_skpd)) == trim($skpd->kode_skpd) ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <select id="bulan" name="bulan"
                        ...
                        <option value="Januari" {{ old('bulan', $bku->bulan) == 'Januari' ? 'selected' : '' }}>Januari</option>
                        <option value="Februari" {{ old('bulan', $bku->bulan) == 'Februari' ? 'selected' : '' }}>Februari</option>
                        <option value="Maret" {{ old('bulan', $bku->bulan) == 'Maret' ? 'selected' : '' }}>Maret</option>
                        <option value="April" {{ old('bulan', $bku->bulan) == 'April' ? 'selected' : '' }}>April</option>
                        <option value="Mei" {{ old('bulan', $bku->bulan) == 'Mei' ? 'selected' : '' }}>Mei</option>
                        <option value="Juni" {{ old('bulan', $bku->bulan) == 'Juni' ? 'selected' : '' }}>Juni</option>
                        <option value="Juli" {{ old('bulan', $bku->bulan) == 'Juli' ? 'selected' : '' }}>Juli</option>
                        <option value="Agustus" {{ old('bulan', $bku->bulan) == 'Agustus' ? 'selected' : '' }}>Agustus</option>
                        <option value="September" {{ old('bulan', $bku->bulan) == 'September' ? 'selected' : '' }}>September</option>
                        <option value="Oktober" {{ old('bulan', $bku->bulan) == 'Oktober' ? 'selected' : '' }}>Oktober</option>
                        <option value="November" {{ old('bulan', $bku->bulan) == 'November' ? 'selected' : '' }}>November</option>
                        <option value="Desember" {{ old('bulan', $bku->bulan) == 'Desember' ? 'selected' : '' }}>Desember</option>
                    </select>
                    @error('bulan')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="tahun" name="tahun" value="{{ old('tahun', $bku->tahun) }}"
